<template>
  ...
  <v-flex v-if="fullSchema && fullSchema.const === undefined && fullSchema['x-display'] !== 'hidden'" :class="propertyClass" :style="fullSchema['x-style'] || ''">
    <slot
      ...
      :required="required"
      :disabled="disabled || fullSchema.disabled"
      :rule="rules"
    ...
    >
      <!-- Date picker -->
      <v-menu
        v-if="fullSchema.type === 'string' && ['date', 'date-time'].includes(fullSchema.format)"
        ref="menu"
        v-model="menu"
      ...
      <div v-else-if="fullSchema.type === 'object' && !fullSchema.key">
        <v-subheader v-if="fullSchema.title" :style="foldable ? 'cursor:pointer; ' :'margin-bottom: -15px'" class="mt-2" @click="folded = !folded">
          {{ fullSchema.title }}
        ...
        <v-slide-y-transition>
          <v-layout v-show="!foldable || !folded" row wrap class="ma-0">
            <p v-if="fullSchema.description" style="margin-bottom: -20px" v-html="htmlDescription" />
            <property
      ...
      <v-row v-else>
        ...
        &nbsp;&nbsp;
          ...
          <!-- Autocomplete field based on a oneOf on a simple type (array or simple value) -->
          <div v-else-if="oneOfSelect">
            <v-subheader style="justify-content: flex-end; height: auto">
              <a v-if="modelWrapper[modelKey]" class="details" :href="fkLink" target="_blank">{{ $t('details') }}</a>
            </v-subheader>
            <v-autocomplete
              v-model="modelWrapper[modelKey]"
              :items="selectItems"
          ...
          <v-select
            v-else-if="fullSchema['x-display'] !== 'list' && (fromUrl || fullSchema['x-fromData'])"
            v-model="modelWrapper[modelKey]"
            :items="selectItems"
            :name="fullKey"
          ...
          <div v-else-if="fromUrlWithQuery">
            <v-subheader style="justify-content: flex-end; height: auto">
              <a v-if="modelWrapper[modelKey]" class="details" :href="fkLink" target="_blank">{{ $t('details') }}</a>
            </v-subheader>
            <v-autocomplete
          ...
          <crudility-input
            v-else-if="fullSchema.type === 'string' && fullSchema['x-display'] === 'password'"
            v-model="modelWrapper[modelKey]"
            :name="fullKey"
            :disabled="disabled || fullSchema.disabled"
            :required="required"
          ...
          <!-- Script field -->
        <codemirror
          v-else-if="fullSchema.type === 'script'"
          v-model="modelWrapper[modelKey]"
          :name="fullKey"
          :options="cmOptions"
          :class="disabled ? 'codemirror--read-only' : ''"
          @change="change"
          @input="input"
        ></codemirror>
          ...
          <div
            v-else-if="fullSchema['x-display'] === 'slider' && (fullSchema.type === 'number' || fullSchema.type === 'integer')"
            class="row vjsf-line"
          >
            <v-slider
          ...
          <div
            v-else-if="fullSchema.type === 'number' || fullSchema.type === 'integer'"
            class="row vjsf-line"
          >
            <crudility-input
              v-model.number="modelWrapper[modelKey]"
          ...
          <div
            v-else-if="fullSchema.type === 'long'"
            class="row vjsf-line"
          >
            <crudility-input
              v-model="modelWrapper[modelKey]"
        ...
        &nbsp;
          ...
          <!-- Simple table with link -->
        <div
        v-else-if="fullSchema.type === 'table'"
          class="vjsf-line"
        >
          <template>
            <v-subheader style="justify-content: flex-end">
              <a v-if="fullSchema.link" class="details" :href="fullSchema.link" target="_blank">{{ $t('details') }}</a>
            </v-subheader>
            <v-data-table
              :headers="fullSchema.columns"
              :items="modelWrapper[modelKey] || []"
              :search="modelWrapper[modelKey + 'Search']"
              disable-sort
              class="elevation-1"
              hide-default-footer
            >
              <template v-slot:items="props">
                <td v-for="column in fullSchema.columns" :key="column.value">
...
<script>
import schemaUtils from '../../../assets/js/vuetify-jsonschema-form/schema'
import selectUtils from '../../../assets/js/vuetify-jsonschema-form/select'
import crudilityUtils from '../../../assets/js/vuetify-jsonschema-form/crudility'
import serverApi from '../../../api'
import SelectIcon from './SelectIcon.vue'
import SelectItem from './SelectItem.vue'
import Tooltip from './Tooltip.vue'
import CrudilityInput from '../../ui/CrudilityInput'
import CrudilityCheckbox from '../../ui/CrudilityCheckbox'
...
export default {
  ...
  computed: {
    ...
    fromUrl () {
      return !!(this.fullSchema['x-fromUrl'] && !this.fullSchema['x-fromUrl'].includes('{q}'))
    },
    fromUrlWithQuery () {
      return !!(this.fullSchema['x-fromUrl'] && this.fullSchema['x-fromUrl'].includes('{q}'))
    },
    ...
    oneOfConstProp () {
      ...
      if (!key) { return }
      return Object.assign(props[key], { key, htmlDescription: md.render(props[key].description || '') })
    },
    oneOfRequired () {
    ...
    fkLink () {
      return this.fullSchema.link + this.modelWrapper[this.modelKey]
    },
    cmOptions () {
      return {
        tabSize: 4,
  ...
  methods: {
    /**
     * Added processing of data downloaded from the crudility ap:
     * - add the current item to the list of items 'crudilityUtils.addCurrentItem'
     * - display list items in displayTemplate format 'crudilityUtils.transformItemsForAutocomplete'
     */
    updateSelectItems () {
      let selectItems = selectUtils.getSelectItems(this.rawSelectItems, this.fullSchema, this.modelWrapper, this.modelKey, this.itemKey)
      ...
      } else if (crudilityUtils.isItemsLoadedFromQuery(this.fullSchema, this.rawSelectItems)) {
        crudilityUtils.addCurrentItem(this.modelWrapper, this.modelKey, selectItems, this.itemKey, this.options.context.initialData)
        selectItems = crudilityUtils.transformItemsForAutocomplete(selectItems, this.fullSchema.displayTemplate)
      } else {
    ...
    /**
     *  Method implementation changed to load entities by query from Crudility API
     *  httpLib.get changed to serverApi.fetchData
     */
    fetchSelectItems () {
      let url = this.fullSchema['x-fromUrl'].replace('{q}', '')
      if (crudilityUtils.IsFilled(this.q)) {
        url += crudilityUtils.buildQueryConditions(this.options.context.entitySchema, this.q, this.fullSchema.entityName)
      }
      for (const key of this.fromUrlKeys) {
        // URL parameters are incomplete
        if (this.fromUrlParams[key] === undefined) { return } else { url = url.replace(`{${key}}`, this.fromUrlParams[key]) }
      }
      this.loading = true
      serverApi.fetchData({ query: url })
        .then((res) => {
          const body = res.data || res.body
          const items = this.fullSchema['x-itemsProp'] ? body[this.fullSchema['x-itemsProp']] : body
          if (!Array.isArray(items)) { throw new TypeError(`Result of http fetch ${url} is not an array`) }
    ...
    initFromSchema () {
      ...
      // const always wins
      if (this.fullSchema.const !== undefined) {
        model = this.fullSchema.const
      }

      // color pickers do not like null values
      if (this.fullSchema.type === 'string' && this.fullSchema.format === 'hexcolor') {
        model = model || ''
      }
...
.codemirror--read-only .CodeMirror {
